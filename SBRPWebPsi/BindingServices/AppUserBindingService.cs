...
namespace SBRPWebPsi.BindingServices
{
    public class AppUserBindingService
    {
        ...
        //public string? GetWebToken(IdentityClaimEntity _loginClaimInfo)
        //{
        //    return m_OrgUserService.GetWebToken(_loginClaimInfo.UserSID, _loginClaimInfo.LoginActionSN);
        //}
        //public async Task<string?> GetWebTokenAsync(IdentityClaimEntity _loginClaimInfo)
        //{
        //    return await m_OrgUserService.GetWebTokenAsync(_loginClaimInfo.UserSID, _loginClaimInfo.LoginActionSN);
        //}
        ...
        //public await IQueryable<AppUser> GetQuery(IEnumerable<short> _userNoEnumer)
        //{
        //    return m_AppUserRepository.GetQuery(_userNoEnumer);
        //}


        public AppUserViewModel? GetEntity(short _userNo, bool _enableTracking = false, bool _includeDetails = true)
        {
            return m_Mapper.Map<AppUserViewModel>(
                m_AppUserService.GetEntity(_userNo, _enableTracking, _includeDetails)
                );
        }
        public async Task<AppUserViewModel?> GetEntityAsync(short _userNo, bool _enableTracking = false, bool _includeDetails = true)
        {
            return m_Mapper.Map<AppUserViewModel>(
        ...
        public AppUserViewModel? GetEntity(string _loginId, string _passwordHash)
        {
            return m_Mapper.Map<AppUserViewModel>(
                m_AppUserService.GetEntity(_loginId, _passwordHash)
                );
        }
        public async Task<AppUserViewModel?> GetEntityAsync(string _loginId, string _passwordHash)
        {
            return m_Mapper.Map<AppUserViewModel>(
