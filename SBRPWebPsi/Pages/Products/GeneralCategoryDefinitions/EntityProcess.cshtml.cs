...
namespace SBRPWebPsi.Pages.Products.GeneralCategoryDefinitions
{
    [Authorize]
    public class EntityProcessModel : PageModel
    {
        private const string m_PageId = "PTGC0105";
        ...
        [BindProperty]
        public ProductGeneralCategoryDefinitionViewModel PG_Info { get; set; }
        ...
        private async Task PG_Info_ReloadAsync(byte? _no, OperationEntityLoadingPolicyEnum _loadingPolicy = default(byte))
        {
            // 考量從其他頁面直接呼叫此程序，或者HTML(disabled)無法POST回正確的db value ，故強制Reload
            if (PG_Info != null && PG_Info.PGCategoryNo.IsNullOrDefault() == false)
            {
                PG_No = PG_Info.PGCategoryNo;
            }
            else
            {
                PG_No = _no.ToByte();
            ...
            if (PG_Info == null)
                PG_Info = await m_ProductGeneralCategoryDefinitionBindingService.GetEntityAsync(PG_No, _enableTracking: false, _includeDetails: true);
        ...
        public async Task OnGetAsync(byte _no, OperationEntityLoadingPolicyEnum _loadingPolicy = default(byte))
        {
            var currentFormEditMode = FormEditModeEnum.Read;
            await Page_InitialAsync(currentFormEditMode);

            PG_Info = await m_ProductGeneralCategoryDefinitionBindingService.GetEntityAsync(_no, _enableTracking:false, _includeDetails:true);
            PG_No = PG_Info.PGCategoryNo;
        ...
        public async Task OnGetAddAsync()
        {
            ...
            PG_Info = m_ProductGeneralCategoryDefinitionBindingService.AddNewDefault();
        ...
        public async Task OnGetRemoveAsync(byte _no)
        {
        ...
        public async Task<IActionResult> OnPostInsertAsync()
        {
            ...
            // -------------------------------------------------------
            PG_Info.PGCategoryId = PG_Info.PGCategoryId.ToUpper();



            // ================================================================
            var result = await m_ProductGeneralCategoryDefinitionBindingService.ProcessToInsertAsync(PG_Info);
        ...
        public async Task<IActionResult> OnPostDeleteAsync(byte _no)
        {
            ...
            var validation = m_ProductGeneralCategoryDefinitionBindingService.IsValidEntity(PG_Info, currentFormEditMode);
            ...
            ////  ========================================================================
            var result = await m_ProductGeneralCategoryDefinitionBindingService.ProcessToDeleteAsync(PG_Info);
            if (result.HasError)
            {
